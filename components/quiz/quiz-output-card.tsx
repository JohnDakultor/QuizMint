    ...
    >
      ...
      <CardHeader className="relative pb-3">
        <div className="flex flex-wrap gap-2 mt-3">
          <Button id="quiz-copy-output" size="sm" variant="outline" onClick={onCopy} className="border-zinc-300 bg-white text-zinc-800 hover:bg-zinc-50 dark:border-slate-500/70 dark:bg-slate-900/55 dark:text-slate-100 dark:hover:bg-slate-800">
          ...
          <Button id="quiz-download-pdf" size="sm" onClick={onDownloadPDF} className="bg-rose-600 text-white hover:bg-rose-700">
            PDF
          </Button>
          <Button id="quiz-download-word" size="sm" onClick={onDownloadWord} className="bg-blue-600 text-white hover:bg-blue-700">
            Word
          </Button>
          <Button id="quiz-download-ppt" size="sm" onClick={onDownloadPPT} className="bg-amber-500 text-slate-950 hover:bg-amber-400">
            PPT
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent className="premium-scrollbar flex-1 min-h-0 overflow-y-auto space-y-4 pr-2">
        {loading ? (
          ...
          quiz.questions.map((q: any, i: number) => (
            <div key={i} className="rounded-lg border border-zinc-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/55">
              {(() => {
              ...
              {q.explanation && <p className="mt-1 text-sm text-zinc-600 dark:text-slate-300">Explanation: {q.explanation}</p>}
            </div>
          ))
        ) : (
          <p className="mt-20 text-center text-zinc-400 dark:text-slate-400">Your generated quiz will appear here</p>
        )}
      </CardContent>

      {quiz && (
        <div className="shrink-0 border-t border-zinc-200 bg-zinc-50/70 p-3 dark:border-slate-700 dark:bg-slate-900/70">
          <details id="quiz-references" className="rounded-md border border-cyan-300/60 bg-white p-2 shadow-[0_0_0_1px_rgba(34,211,238,0.15)] dark:border-cyan-400/35 dark:bg-slate-950/70">
            ...
            <div className="mt-2 max-h-24 overflow-y-auto premium-scrollbar pr-1">
              {sources.length > 0 ? (
                ...
                <div className="text-xs text-zinc-600 dark:text-slate-300">No website reference detected for this quiz.</div>
              )}
            </div>
          </details>
