...
//   deleteDeckSlide,
//   fromCanvasPixels,
//   getCanvasBodyFontSize,
//   getCanvasFontFamily,
//   getCanvasLayout,
//   getCanvasTitleFontSize,
...
// type CanvasObjectKey = CanvasZoneKey | `bullet-${number}` | `visual-item-${string}` | `body-block-${number}`;

// function isCanvasZoneKey(key: CanvasObjectKey | null): key is CanvasZoneKey {
//   return (
//     Boolean(key) &&
...
//       type="button"
//       onClick={onClick}
//       className={`group flex h-[236px] flex-col rounded-2xl border p-3 text-left transition ${
//         active
//           ? "border-blue-500 bg-white shadow-[0_0_0_2px_rgba(59,130,246,0.14),0_12px_28px_rgba(15,23,42,0.10)] dark:border-cyan-400 dark:bg-slate-800/90 dark:shadow-[0_0_0_1px_rgba(34,211,238,0.18),0_18px_42px_rgba(2,8,23,0.45)]"
//           : "border-slate-200/90 bg-white hover:border-blue-300 hover:bg-slate-50 dark:border-white/10 dark:bg-slate-900/60 dark:hover:border-cyan-300/40 dark:hover:bg-slate-900/85"
//       }`}
//     >
...
//         <div className={`h-[126px] space-y-2 overflow-hidden p-3 ${fontClass}`}>
...
//                   style={{ width: `${Math.max(38, 82 - bulletIndex * 12)}%` }}
//                 />
//               ))
//             ) : (
//               <>
...
//   selectedBlock: CanvasObjectKey | null;
//   freeMove: boolean;
//   snapEnabled: boolean;
//   onSelectBlock: (block: CanvasObjectKey | null) => void;
//   onLayoutChange: (block: CanvasObjectKey, zone: CanvasZone) => void;
//   onReorderBlock: (block: CanvasObjectKey, direction: "forward" | "backward") => void;
//   onRemoveBlock: (block: CanvasObjectKey) => void;
//   onEditBlock: (block: CanvasObjectKey, value: string) => void;
// }) {
//   const containerRef = useRef<HTMLDivElement | null>(null);
//   const [stageSize, setStageSize] = useState({ width: 1080, height: 675 });
...
//   const [contextMenu, setContextMenu] = useState<{ x: number; y: number; block: CanvasObjectKey } | null>(null);
//   const [inlineEdit, setInlineEdit] = useState<{
//     block: CanvasObjectKey;
//     value: string;
//     x: number;
//     y: number;
//     w: number;
...
//     contentGridClass,
//     visualImageClass,
//     visualTextClampClass,
//   } = getPreviewLayoutConfig(slide);
//   void titleClass;
//   void bodyClass;
//   void contentGridClass;
//   void visualImageClass;
//   void visualTextClampClass;
//   const bullets = slide.bullets?.filter(Boolean) || [];
//   const bodyBlocks = slide.bodyBlocks?.filter(Boolean) || [];
//   const visualItems = toVisualItems(slide);
//   const tone = toneForSlide(slide);
//   const subtitle = deck.subtitle || "";
//   const canvasLayout = getCanvasLayout(slide);
//   const stageWidth = stageSize.width;
//   const stageHeight = stageSize.height;
//   const fontFamily = getCanvasFontFamily(slide);
//   const titleFontSize = getCanvasTitleFontSize(slide);
//   const bodyFontSize = getCanvasBodyFontSize(slide);
//   const accentStops = getToneGradientStops(slide.accentTone);
//   const titleBox = toCanvasPixels(canvasLayout.title, stageWidth, stageHeight);
//   const bodyBox = toCanvasPixels(canvasLayout.body, stageWidth, stageHeight);
//   const bulletsBox = toCanvasPixels(canvasLayout.bullets, stageWidth, stageHeight);
//   const visualBox = toCanvasPixels(canvasLayout.visual, stageWidth, stageHeight);
//   const notesBox = toCanvasPixels(canvasLayout.notes, stageWidth, stageHeight);
//   const orderedBlocks = useMemo(() => {
//     const source = slide.canvasOrder?.length ? slide.canvasOrder : DEFAULT_CANVAS_ORDER;
//     const unique = Array.from(new Set(source.filter(Boolean)));
//     return [
//       ...unique.filter((key): key is CanvasZoneKey => DEFAULT_CANVAS_ORDER.includes(key as CanvasZoneKey)),
//       ...DEFAULT_CANVAS_ORDER.filter((key) => !unique.includes(key)),
//     ];
//   }, [slide.canvasOrder]);
//   const visualImageMap = useLoadedImageMap(
//     visualItems.map((item) => ({
//       id: item.id,
//       src: item.imageData || slide.imageData,
//     }))
//   );

//   function clampForCanvas(key: CanvasObjectKey, zone: { x: number; y: number; w: number; h: number }) {
//     const minSize = key.startsWith("bullet-")
//       ? { w: 220, h: 72 }
//       : key.startsWith("visual-item-")
//         ? { w: 220, h: 150 }
//         : key.startsWith("body-block-")
//           ? { w: 220, h: 64 }
//         : { w: 120, h: 56 };
//     if (!freeMove) {
//       const baseKey: CanvasZoneKey = key.startsWith("bullet-")
//         ? "bullets"
//         : key.startsWith("visual-item-")
//           ? "visual"
//           : key.startsWith("body-block-")
//             ? "body"
//           : (key as CanvasZoneKey);
//       const clamped = clampZoneToLane(baseKey, zone, stageWidth, stageHeight);
//       return {
//         ...clamped,
//         w: Math.max(minSize.w, clamped.w),
//         h: Math.max(minSize.h, clamped.h),
//       };
//     }
//     const minW = minSize.w;
//     const minH = minSize.h;
//     const maxW = stageWidth - 24;
//     const maxH = stageHeight - 24;
//     const w = Math.max(minW, Math.min(maxW, zone.w));
//     const h = Math.max(minH, Math.min(maxH, zone.h));
//     const x = Math.max(0, Math.min(stageWidth - w, zone.x));
//     const y = Math.max(12, Math.min(stageHeight - h, zone.y));
//     return { x, y, w, h };
//   }

//   void onRemoveBlock;

//   function scheduleSnapGuides(next: { vertical?: number; horizontal?: number }) {
//     const prev = snapGuidesRef.current;
//     const same = prev.vertical === next.vertical && prev.horizontal === next.horizontal;
//     if (same) return;
...
//     value: string
//   ) {
//     if (!containerRef.current || !stageWrapRef.current) return;
...
//       block === "title"
//         ? titleFontSize
//         : block.startsWith("bullet-")
//           ? Math.min(15, Math.max(12, zone.h * 0.22))
//           : block.startsWith("body-block-")
//             ? Math.max(15, bodyFontSize - 3)
//             : block === "body"
//               ? Math.max(17, bodyFontSize - 3)
//               : Math.max(14, bodyFontSize - 2);
//     const fontWeight = block === "title" || block.startsWith("bullet-") ? 700 : 500;
//     const lineHeight = block === "title" ? 1.05 : 1.2;
//     setInlineEdit({
//       block,
//       value,
//       x: offsetX + zone.x + 6,
//       y: offsetY + zone.y + 6,
//       w: Math.max(140, zone.w - 12),
//       h: Math.max(48, zone.h - 12),
//       fontSize,
//       fontWeight,
//       lineHeight,
//       zone: { ...zone },
...
//   function measureTextHeight(input: {
//     text: string;
//     width: number;
//     fontSize: number;
//     fontWeight: number | string;
...
//     if (!el) return input.fontSize * input.lineHeight * 1.4;
//     el.style.width = `${Math.max(40, input.width)}px`;
...
//   function getBodyBlockZone(index: number) {
//     const rows = Math.max(1, bodyBlocks.length);
//     const fallbackH = Math.max(
//       56,
//       Math.min(90, (bodyBox.h - 12 * Math.max(0, rows - 1)) / rows)
//     );
//     const fallbackZone = {
//       x: bodyBox.x,
//       y: bodyBox.y + index * (fallbackH + 12),
//       w: bodyBox.w,
//       h: fallbackH,
//     };
//     const savedZone = slide.bodyBlockLayouts?.[String(index)];
//     return savedZone ? toCanvasPixels(savedZone, stageWidth, stageHeight) : fallbackZone;
//   }

//   function getBulletZone(index: number) {
//     const isTwoColumn = slide.layoutStyle === "two-column";
//     const columnCount = isTwoColumn ? 2 : 1;
//     const rowCount = Math.max(1, Math.ceil(bullets.length / columnCount));
//     const horizontalGap = isTwoColumn ? 14 : 0;
//     const verticalGap = 14;
//     const fallbackCardW = (bulletsBox.w - horizontalGap * (columnCount - 1)) / columnCount;
//     const fallbackCardH = Math.max(
//       58,
//       Math.min(92, (bulletsBox.h - verticalGap * Math.max(0, rowCount - 1)) / rowCount)
//     );
//     const col = isTwoColumn ? index % 2 : 0;
//     const row = isTwoColumn ? Math.floor(index / 2) : index;
//     const fallbackZone = {
//       x: bulletsBox.x + col * (fallbackCardW + horizontalGap),
//       y: bulletsBox.y + row * (fallbackCardH + verticalGap),
//       w: fallbackCardW,
//       h: fallbackCardH,
//     };
//     const savedZone = slide.bulletLayouts?.[String(index)];
//     return savedZone ? toCanvasPixels(savedZone, stageWidth, stageHeight) : fallbackZone;
//   }

//   function commitInlineEdit(nextValue?: string) {
//     if (!inlineEdit) return;
//     onEditBlock(inlineEdit.block, nextValue ?? inlineEdit.value);
//     setInlineEdit(null);
//   }
...
//     const sync = () => {
//       const availableWidth = Math.max(920, node.clientWidth - 12);
//       const availableHeight = Math.max(680, node.clientHeight - 12);
//       const widthFromHeight = Math.floor((availableHeight * 16) / 10);
//       const nextWidth = Math.min(availableWidth, widthFromHeight);
//       const nextHeight = Math.floor((nextWidth * 10) / 16);
...
//   useEffect(() => {
//     if (!inlineEditRef.current || !inlineEdit) return;
//     const el = inlineEditRef.current;
//     el.style.height = "auto";
//     const maxHeight = Math.max(48, stageHeight - inlineEdit.y - 12);
//     const next = Math.max(inlineEdit.h, Math.min(el.scrollHeight, maxHeight));
//     el.style.height = `${next}px`;
//   }, [inlineEdit, stageHeight]);

//   useEffect(() => {
//     if (!transformerRef.current) return;
//       const node = selectedBlock ? zoneRefs.current[selectedBlock] : null;
//     transformerRef.current.nodes(node ? [node] : []);
//     transformerRef.current.getLayer()?.batchDraw();
//   }, [selectedBlock, stageWidth, stageHeight, slide]);

//   function renderSelectableZone(
//     key: CanvasObjectKey,
//     zone: { x: number; y: number; w: number; h: number },
//     children: ReactNode,
//     onEdit?: () => void
//   ) {
//     const isEditing = inlineEdit?.block === key;
//     const selected = selectedBlock === key && !isEditing;
//     return (
//       <Group
//         key={`zone-${key}`}
//         x={zone.x}
...
//         draggable
//         dragBoundFunc={(pos) => {
//           const clamped = clampForCanvas(key, { x: pos.x, y: pos.y, w: zone.w, h: zone.h });
//           if (!snapEnabled) {
//             return clamped;
//           }
//           const snapped = applySnapGuides(clamped, stageWidth, stageHeight);
//           return snapped.zone;
//         }}
//         onDragMove={(event) => {
//           if (!snapEnabled) return;
//           const clamped = clampForCanvas(key, {
//             x: event.target.x(),
//             y: event.target.y(),
//             w: zone.w,
//             h: zone.h,
//           });
//           const snapped = applySnapGuides(clamped, stageWidth, stageHeight);
//           scheduleSnapGuides(snapped.guides);
//         }}
//         onClick={() => onSelectBlock(key)}
//         onTap={() => onSelectBlock(key)}
//         onDblClick={() => onEdit?.()}
//         onDblTap={() => onEdit?.()}
//         onContextMenu={(event) => {
//           event.evt.preventDefault();
//           onSelectBlock(key);
//           setContextMenu({ x: event.evt.clientX, y: event.evt.clientY, block: key });
//         }}
//         onDragEnd={(event) => {
//           const clamped = clampForCanvas(key, { x: event.target.x(), y: event.target.y(), w: zone.w, h: zone.h });
//           const zoneToPersist = snapEnabled
//             ? applySnapGuides(clamped, stageWidth, stageHeight).zone
//             : clamped;
//           onLayoutChange(key, fromCanvasPixels(zoneToPersist, stageWidth, stageHeight));
//           scheduleSnapGuides({});
//         }}
//       >
//         <Rect
...
//             const next = fromCanvasPixels(
//               clampForCanvas(key, {
//                 x: node.x(),
//                 y: node.y(),
//                 w: Math.max(120, node.width() * scaleX),
//                 h: Math.max(70, node.height() * scaleY),
//               }),
//               stageWidth,
//               stageHeight
//             );
...
//   return (
//     <div className="relative flex h-full min-h-full w-full flex-col bg-[#e9eef5] dark:bg-[#0f172a]">
//       <div
//         ref={containerRef}
//         className="relative flex h-full min-h-0 w-full items-center justify-center overflow-hidden p-3"
//       >
//         <div
//           ref={stageWrapRef}
//           className="relative overflow-hidden bg-white shadow-[0_18px_40px_rgba(15,23,42,0.12)]"
//         >
//           <div
//             ref={measureRef}
//             className="pointer-events-none absolute -left-[9999px] top-0 whitespace-pre-wrap opacity-0"
//             style={{ fontFamily }}
//           />
//           <Stage
//             width={stageWidth}
...
//               const clickedBackground = target?.attrs?.name === "canvas-clear";
...
//                 fillLinearGradientColorStops={[0, accentStops[0], 0.5, accentStops[1], 1, accentStops[2]]}
//               />
//               <Rect name="canvas-clear" x={0} y={12} width={stageWidth} height={stageHeight - 12} fill="#ffffff" />

//               <KonvaText
//                 x={46}
//                 y={50}
//                 text={`SLIDE ${selectedIndex + 1}`}
//                 fontSize={12}
//                 letterSpacing={6}
//                 fill="#94a3b8"
//                 fontStyle="bold"
...
//                   x={18}
//                   y={10}
//                   text={prettifyLayout(slide.layoutStyle)}
//                   fontSize={14}
//                   fill="#6366f1"
//                   fontStyle="bold"
//                   fontFamily={fontFamily}
...
//                 if (block === "title") {
//                   return renderSelectableZone(
//                     "title",
//                     titleBox,
//                     inlineEdit?.block === "title" ? null : (
//                       <KonvaText
//                         x={14}
//                         y={10}
//                         width={titleBox.w - 24}
//                         height={titleBox.h - 12}
...
//                       />
//                     ),
//                     () => beginInlineEdit("title", titleBox, slide.title || "")
//                   );
//                 }
...
//                     );
//                     return (
//                       <Group key="zone-group-body-blocks">
//                         {bodyBlocks.map((line, index) => {
//                           const fallbackZone = {
//                             x: bodyBox.x,
//                             y: bodyBox.y + index * (fallbackH + 12),
//                             w: bodyBox.w,
...
//                             ? toCanvasPixels(savedZone, stageWidth, stageHeight)
//                             : fallbackZone;
//                           const blockKey = `body-block-${index}` as const;
//                           return renderSelectableZone(
//                             blockKey,
//                             blockZone,
//                             inlineEdit?.block === blockKey ? (
//                               <Rect
//                                 width={blockZone.w}
//                                 height={blockZone.h}
//                                 cornerRadius={16}
...
//                                   x={16}
//                                   y={12}
//                                   width={blockZone.w - 26}
//                                   height={blockZone.h - 18}
//                                   text={line}
//                                   fontSize={Math.max(15, bodyFontSize - 3)}
//                                   fill="#475569"
//                                   lineHeight={1.18}
//                                   wrap="word"
//                                   ellipsis
...
//                               </>
//                             ),
//                             () => beginInlineEdit(blockKey, blockZone, line)
//                           );
//                         })}
//                       </Group>
//                     );
...
//                     "body",
//                     bodyBox,
//                     inlineEdit?.block === "body" ? null : (
//                       <>
//                         {(bodyBlocks.length > 0 ? bodyBlocks : [slide.body || "Add supporting body copy here."]).map(
//                           (line, index) => (
//                             <KonvaText
//                               key={`body-line-${index}`}
//                               x={14}
//                               y={10 + index * (bodyFontSize + 10)}
//                               width={bodyBox.w - 24}
//                               height={Math.min(bodyBox.h - 12, bodyFontSize * 4)}
//                               text={line}
//                               fontSize={
//                                 bullets.length > 0
//                                   ? Math.max(15, bodyFontSize - 4)
//                                   : Math.max(17, bodyFontSize - 3)
//                               }
//                               fill="#475569"
//                               lineHeight={1.18}
...
//                       </>
//                     ),
//                     () => beginInlineEdit("body", bodyBox, slide.body || "")
//                   );
//                 }
...
//                   const verticalGap = 14;
//                   const fallbackCardW =
//                     (bulletsBox.w - horizontalGap * (columnCount - 1)) / columnCount;
//                   const fallbackCardH = Math.max(
//                     58,
//                     Math.min(
//                       92,
//                       (bulletsBox.h - verticalGap * Math.max(0, rowCount - 1)) / rowCount
//                     )
//                   );
//                   return (
//                     <Group key="zone-group-bullets">
//                       {bullets.map((bullet, bulletIndex) => {
//                         const col = isTwoColumn ? bulletIndex % 2 : 0;
//                         const row = isTwoColumn ? Math.floor(bulletIndex / 2) : bulletIndex;
//                         const fallbackZone = {
//                           x: bulletsBox.x + col * (fallbackCardW + horizontalGap),
//                           y: bulletsBox.y + row * (fallbackCardH + verticalGap),
//                           w: fallbackCardW,
//                           h: fallbackCardH,
//                         };
//                         const savedZone = slide.bulletLayouts?.[String(bulletIndex)];
//                         const bulletZone = savedZone
//                           ? toCanvasPixels(savedZone, stageWidth, stageHeight)
//                           : fallbackZone;
//                         const bulletKey = `bullet-${bulletIndex}` as const;
//                         return renderSelectableZone(
//                           bulletKey,
//                           bulletZone,
//                           inlineEdit?.block === bulletKey ? (
//                             <Rect
//                               width={bulletZone.w}
//                               height={bulletZone.h}
//                               cornerRadius={18}
...
//                                 x={18}
//                                 y={14}
//                                 width={bulletZone.w - 30}
//                                 height={bulletZone.h - 18}
//                                 text={bullet}
//                                 fontSize={Math.min(15, Math.max(12, bulletZone.h * 0.22))}
//                                 fontStyle="bold"
//                                 fill="#334155"
//                                 wrap="word"
//                                 ellipsis
...
//                             </>
//                           ),
//                           () => beginInlineEdit(bulletKey, bulletZone, bullet)
//                         );
//                       })}
//                     </Group>
//                   );
...
//                 if (block === "visual") {
//                   const visualCards = visualItems.length > 0 ? visualItems : [{ id: "placeholder", prompt: "" }];
//                   return (
//                     <Group key="zone-group-visual">
//                       {visualCards.map((item, itemIndex) => {
//                         const savedZone = slide.visualItemLayouts?.[item.id];
//                         const itemImage = visualImageMap[item.id] || null;
//                         const defaultZone = {
//                           x: visualBox.x + 22,
//                           y: visualBox.y + 22 + itemIndex * 184,
//                           w: visualBox.w - 44,
//                           h: Math.max(150, Math.min(176, visualBox.h / Math.max(1, visualCards.length) - 18)),
//                         };
//                         const itemZone = savedZone
//                           ? toCanvasPixels(savedZone, stageWidth, stageHeight)
//                           : defaultZone;
//                         const itemKey = `visual-item-${item.id}` as const;
//                         const imageHeight = itemImage ? itemZone.h : Math.max(96, Math.min(itemZone.h - 20, itemZone.h * 0.68));
...
//                                   cornerRadius={24}
//                                   fillLinearGradientStartPoint={{ x: 0, y: 0 }}
//                                   fillLinearGradientEndPoint={{ x: itemZone.w, y: itemZone.h }}
...
//                                   y={imageHeight + 20}
//                                   width={itemZone.w - 56}
//                                   height={Math.max(34, itemZone.h - imageHeight - 46)}
//                                   text="Image from prompt"
//                                   fontSize={14}
//                                   fill="#64748b"
//                                   lineHeight={1.15}
//                                   wrap="word"
...
//               {snapEnabled && snapGuides.vertical ? <Rect x={snapGuides.vertical} y={24} width={1} height={stageHeight - 48} fill="#38bdf8" opacity={0.65} /> : null}
//               {snapEnabled && snapGuides.horizontal ? <Rect x={24} y={snapGuides.horizontal} width={stageWidth - 48} height={1} fill="#38bdf8" opacity={0.65} /> : null}
...
//                   if (newBox.width < 120 || newBox.height < 56) return oldBox;
//                   return newBox;
//                 }}
//               />
//             </Layer>
...
//               });
//               const nextZoneH = Math.max(56, measured + 18);
//               const nextZone = { ...current.zone, h: nextZoneH + 12 };
//               const delta = nextZone.h - current.zone.h;
//               onLayoutChange(current.block, fromCanvasPixels(nextZone, stageWidth, stageHeight));
//               if (delta !== 0) {
//                 if (current.block.startsWith("body-block-")) {
//                   const index = Number(current.block.replace("body-block-", ""));
//                   for (let i = 0; i < bodyBlocks.length; i += 1) {
//                     if (i === index) continue;
//                     const zone = getBodyBlockZone(i);
//                     if (zone.y > current.zone.y) {
//                       const updated = { ...zone, y: zone.y + delta };
//                       onLayoutChange(
//                         `body-block-${i}`,
//                         fromCanvasPixels(updated, stageWidth, stageHeight)
//                       );
//                     }
//                   }
//                 }
//                 if (current.block.startsWith("bullet-")) {
//                   const index = Number(current.block.replace("bullet-", ""));
//                   for (let i = 0; i < bullets.length; i += 1) {
//                     if (i === index) continue;
//                     const zone = getBulletZone(i);
//                     if (zone.y > current.zone.y) {
//                       const updated = { ...zone, y: zone.y + delta };
//                       onLayoutChange(
//                         `bullet-${i}`,
...
//             }
//           }}
//           className="absolute z-[130] resize-none overflow-hidden rounded-lg border border-transparent bg-transparent p-2 text-slate-900 outline-none"
//           style={{
//             left: inlineEdit.x,
//             top: inlineEdit.y,
//             width: inlineEdit.w,
...
//             lineHeight: inlineEdit.lineHeight,
//             fontFamily,
//           }}
//         />
//       ) : null}
//       {contextMenu ? (
//         <div
//           className="fixed z-[120] min-w-[180px] rounded-xl border border-slate-200 bg-white p-2 shadow-2xl dark:border-slate-700 dark:bg-slate-900"
//           style={{ left: contextMenu.x + 8, top: contextMenu.y + 8 }}
//         >
//           <button
//             type="button"
//             onClick={() => {
//               onReorderBlock(contextMenu.block, "forward");
//               setContextMenu(null);
//             }}
...
//             type="button"
//             onClick={() => {
//               onReorderBlock(contextMenu.block, "backward");
//               setContextMenu(null);
//             }}
//             className="block w-full rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
//           >
...
//             type="button"
//             onClick={() => {
//               onRemoveBlock(contextMenu.block);
//               setContextMenu(null);
//             }}
...
//       ) : null}
//       {selectedBlock ? (
//         <div className="absolute right-6 top-16 z-20 flex gap-2 rounded-xl border border-slate-200 bg-white/95 p-2 shadow-lg dark:border-slate-700 dark:bg-slate-900/95">
//           <Button
//             type="button"
//             variant="outline"
//             size="sm"
//             onClick={() => onReorderBlock(selectedBlock, "backward")}
//             className="h-8 rounded-md border-slate-300 bg-white px-2 text-xs dark:border-slate-600 dark:bg-slate-800"
//           >
//             Send Back
//           </Button>
//           <Button
...
//             variant="outline"
//             size="sm"
//             onClick={() => onReorderBlock(selectedBlock, "forward")}
//             className="h-8 rounded-md border-slate-300 bg-white px-2 text-xs dark:border-slate-600 dark:bg-slate-800"
//           >
//             Bring Front
//           </Button>
...
//   const [selectedCanvasBlock, setSelectedCanvasBlock] = useState<CanvasObjectKey | null>(null);
...
//     if (Array.isArray(draftSlide.visualItems) && draftSlide.visualItems.length > 0) {
//       return draftSlide.visualItems;
//     }
//     const prompts = (draftSlide.imagePrompt || "")
//       .split(/\n+/)
//       .map((value) => value.trim())
//       .filter(Boolean);
//     return prompts.map((prompt, index) => ({
...
//   function createVisualItemId() {
//     if (typeof crypto !== "undefined" && "randomUUID" in crypto) {
//       return crypto.randomUUID();
//     }
//     return `visual-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
//   }

//   useEffect(() => {
//     const safeIndex = Math.min(selectedIndex, Math.max(0, deck.slides.length - 1));
//     if (safeIndex !== selectedIndex) {
//       setSelectedIndex(safeIndex);
//       return;
//     }

//     const nextSlide = normalizeSlide(deck.slides[safeIndex] || createBlankSlide(deck));
//     if (skipDraftReloadRef.current) {
//       skipDraftReloadRef.current = false;
//       return;
//     }
//     setDraftSlide((current) => (slidesEqual(current, nextSlide) ? current : nextSlide));
//   }, [deck, selectedIndex]);

//   const selectedCanvasLabel = selectedCanvasBlock?.startsWith("bullet-")
//     ? "Bullet Card"
//     : selectedCanvasBlock?.startsWith("visual-item-")
//       ? "Image Card"
//       : selectedCanvasBlock?.startsWith("body-block-")
//         ? "Body Card"
//       : isCanvasZoneKey(selectedCanvasBlock)
//         ? labelForCanvasBlock(selectedCanvasBlock)
//         : null;
//   const toolOptions = useMemo(
//     () => [
//       {
//         value: "toggle_free_move",
//         label: freeMoveMode ? "Turn Free Move Off" : "Turn Free Move On",
//         requiresSelection: false,
//       },
//       {
//         value: "toggle_snap",
//         label: snapEnabled ? "Turn Snap Off" : "Turn Snap On",
//         requiresSelection: false,
//       },
//       { value: "new_slide", label: "New Slide", requiresSelection: false },
//       { value: "duplicate_slide", label: "Duplicate Slide", requiresSelection: false },
//       { value: "reset_slide", label: "Reset Slide", requiresSelection: false },
//       { value: "delete_slide", label: "Delete Slide", requiresSelection: false },
//       { value: "bring_front", label: "Bring Selection Front", requiresSelection: true },
//       { value: "send_back", label: "Send Selection Back", requiresSelection: true },
//       { value: "reset_zone", label: "Reset Selected Zone", requiresSelection: true },
//       { value: "deselect", label: "Deselect Object", requiresSelection: true },
//     ],
//     [freeMoveMode, snapEnabled]
//   );
//   const selectedToolMeta = toolOptions.find((option) => option.value === toolAction) || null;
//   const canApplyTool =
//     !!toolAction &&
//     (!selectedToolMeta?.requiresSelection || Boolean(selectedCanvasBlock));
...
//   useEffect(() => {
//     if (selectedToolMeta?.requiresSelection && !selectedCanvasBlock) {
//       setToolAction("");
//     }
//   }, [selectedCanvasBlock, selectedToolMeta]);

//   useEffect(() => {
//     if (typeof window !== "undefined") {
//       const konvaRuntime = (window as Window & {
//         Konva?: { pixelRatio: number; _fixTextRendering?: boolean };
//       }).Konva;
//       if (konvaRuntime) {
//         konvaRuntime.pixelRatio = Math.max(window.devicePixelRatio || 1, 2);
//         konvaRuntime._fixTextRendering = true;
//       }
//     }
//   }, []);

//   function updateDraft(patch: Partial<EditablePptSlide>) {
//     setDraftSlide((current) => normalizeSlide({ ...current, ...patch }));
//   }

//   function commitDraftToDeck(baseDeck: PptDeck = deck) {
//     const currentDeckSlide = normalizeSlide(baseDeck.slides[selectedIndex] || createBlankSlide(baseDeck));
//     if (slidesEqual(currentDeckSlide, draftSlide)) {
//       return baseDeck;
//     }
//     return replaceDeckSlide(baseDeck, selectedIndex, draftSlide);
//   }

//   function applyDeckUpdate(nextDeck: PptDeck, options?: { nextIndex?: number; nextDraft?: EditablePptSlide }) {
//     const nextIndex = options?.nextIndex ?? selectedIndex;
//     const nextDraft = options?.nextDraft ?? normalizeSlide(nextDeck.slides[nextIndex] || createBlankSlide(nextDeck));
//     skipDraftReloadRef.current = true;
//     onChange(nextDeck);
//     setSelectedIndex(nextIndex);
//     setDraftSlide(nextDraft);
//   }

//   function handleSelectSlide(nextIndex: number) {
//     if (nextIndex === selectedIndex) return;
//     const committedDeck = commitDraftToDeck(deck);
//     applyDeckUpdate(committedDeck, {
//       nextIndex,
//       nextDraft: normalizeSlide(committedDeck.slides[nextIndex] || createBlankSlide(committedDeck)),
//     });
//   }

//   function handleAddSlide() {
//     const committedDeck = commitDraftToDeck(deck);
//     const nextSlide = createBlankSlide(committedDeck);
//     const nextDeck = addDeckSlide(committedDeck, nextSlide);
//     applyDeckUpdate(nextDeck, {
//       nextIndex: nextDeck.slides.length - 1,
//       nextDraft: normalizeSlide(nextSlide),
//     });
//   }

//   function handleDuplicateSlide() {
//     const nextDeck = cloneDeckSlide(commitDraftToDeck(deck), selectedIndex);
//     const clone = normalizeSlide(nextDeck.slides[selectedIndex + 1]);
//     applyDeckUpdate(nextDeck, {
...
//     const nextDeck = deleteDeckSlide(commitDraftToDeck(deck), selectedIndex);
//     const slides = nextDeck.slides;
//     const nextIndex = Math.max(0, selectedIndex - 1);
//     applyDeckUpdate(nextDeck, {
//       nextIndex,
//       nextDraft: normalizeSlide(slides[nextIndex]),
//     });
//   }

//   function handleResetSlide() {
//     setDraftSlide(normalizeSlide(deck.slides[selectedIndex]));
//   }

//   function handleCanvasLayoutChange(block: CanvasObjectKey, zone: CanvasZone) {
//     if (block.startsWith("bullet-")) {
//       const bulletIndex = block.replace("bullet-", "");
//       updateDraft({
//         bulletLayouts: {
//           ...(draftSlide.bulletLayouts || {}),
//           [bulletIndex]: zone,
//         },
//       });
//       return;
...
//       const blockIndex = block.replace("body-block-", "");
//       updateDraft({
//         bodyBlockLayouts: {
//           ...(draftSlide.bodyBlockLayouts || {}),
//           [blockIndex]: zone,
//         },
//       });
//       return;
...
//     updateDraft({
//       canvasLayout: {
//         ...(draftSlide.canvasLayout || {}),
//         [block]: zone,
//       },
//     });
//   }

//   function handleCanvasReorder(block: CanvasObjectKey, direction: "forward" | "backward") {
//     const delta = direction === "forward" ? 1 : -1;

//     if (block.startsWith("bullet-")) {
//       // Move the entire bullets layer forward/backward relative to visual layer.
//       updateDraft({
//         canvasOrder: moveCanvasBlock(draftSlide.canvasOrder, "bullets", direction),
//       });
//       const index = Number(block.replace("bullet-", ""));
//       const bullets = draftSlide.bullets || [];
//       if (!Number.isInteger(index) || index < 0 || index >= bullets.length) return;
//       const order = bullets.map((_, i) => i);
//       const target = Math.max(0, Math.min(order.length - 1, index + delta));
//       if (target === index) return;
//       const [moved] = order.splice(index, 1);
//       order.splice(target, 0, moved);
//       const nextBullets = order.map((i) => bullets[i]);
//       const nextLayouts: Record<string, CanvasZone> = {};
//       order.forEach((oldIdx, newIdx) => {
//         const layout = draftSlide.bulletLayouts?.[String(oldIdx)];
//         if (layout) nextLayouts[String(newIdx)] = layout;
//       });
//       updateDraft({
//         bullets: nextBullets,
//         bulletLayouts: nextLayouts,
//       });
//       setSelectedCanvasBlock(`bullet-${target}`);
//       return;
//     }

//     if (block.startsWith("body-block-")) {
//       updateDraft({
//         canvasOrder: moveCanvasBlock(draftSlide.canvasOrder, "body", direction),
//       });
//       const index = Number(block.replace("body-block-", ""));
//       const blocks = draftSlide.bodyBlocks || [];
...
//       const [moved] = order.splice(index, 1);
//       order.splice(target, 0, moved);
//       const nextBlocks = order.map((i) => blocks[i]);
//       const nextLayouts: Record<string, CanvasZone> = {};
//       order.forEach((oldIdx, newIdx) => {
//         const layout = draftSlide.bodyBlockLayouts?.[String(oldIdx)];
//         if (layout) nextLayouts[String(newIdx)] = layout;
//       });
//       updateDraft({
//         bodyBlocks: nextBlocks,
//         body: nextBlocks.join(" ").trim(),
//         bodyBlockLayouts: nextLayouts,
//       });
//       setSelectedCanvasBlock(`body-block-${target}`);
//       return;
//     }

//     if (block.startsWith("visual-item-")) {
//       // Move the entire visual layer forward/backward relative to bullets layer.
//       updateDraft({
//         canvasOrder: moveCanvasBlock(draftSlide.canvasOrder, "visual", direction),
//       });
//       const items = Array.isArray(draftSlide.visualItems) && draftSlide.visualItems.length > 0
//         ? [...draftSlide.visualItems]
//         : toVisualItems(draftSlide);
//       const index = items.findIndex((item) => `visual-item-${item.id}` === block);
//       if (index < 0) return;
...
//       setSelectedCanvasBlock(`visual-item-${items[target].id}`);
//       return;
//     }

//     if (!isCanvasZoneKey(block)) {
//       return;
//     }
//     updateDraft({
//       canvasOrder: moveCanvasBlock(draftSlide.canvasOrder, block, direction),
//     });
//   }

//   function handleRemoveCanvasObject(block: CanvasObjectKey) {
//     if (block.startsWith("bullet-")) {
//       const index = Number(block.replace("bullet-", ""));
...
//     if (block.startsWith("body-block-")) {
//       const index = Number(block.replace("body-block-", ""));
//       const nextBlocks = (draftSlide.bodyBlocks || [draftSlide.body || ""]).filter(
//         (_, idx) => idx !== index
//       );
//       const nextLayouts = { ...(draftSlide.bodyBlockLayouts || {}) };
//       delete nextLayouts[String(index)];
//       Object.keys(nextLayouts).forEach((key) => {
//         const numeric = Number(key);
//         if (Number.isInteger(numeric) && numeric > index) {
//           nextLayouts[String(numeric - 1)] = nextLayouts[key];
...
//         visualItemLayouts: nextLayouts,
//         imagePrompt: nextItems.map((item) => item.prompt).join("\n"),
//       });
//       setSelectedCanvasBlock(null);
//       return;
//     }
...
//   function handleEditBlock(block: CanvasObjectKey, value: string) {
//     if (block.startsWith("bullet-")) {
//       const index = Number(block.replace("bullet-", ""));
...
//     if (block.startsWith("body-block-")) {
//       const index = Number(block.replace("body-block-", ""));
//       const nextBlocks = [...(draftSlide.bodyBlocks || [])];
//       nextBlocks[index] = value;
//       updateDraft({
//         bodyBlocks: nextBlocks,
//         body: nextBlocks.filter(Boolean).join(" "),
//       });
//       return;
//     }
...
//   function handleResetSelectedZone() {
//     if (!selectedCanvasBlock) return;
//     if (selectedCanvasBlock.startsWith("bullet-")) {
//       const bulletIndex = selectedCanvasBlock.replace("bullet-", "");
//       const nextLayouts = { ...(draftSlide.bulletLayouts || {}) };
//       delete nextLayouts[bulletIndex];
//       updateDraft({ bulletLayouts: nextLayouts });
//       return;
//     }
//     if (selectedCanvasBlock.startsWith("body-block-")) {
//       const blockIndex = selectedCanvasBlock.replace("body-block-", "");
//       const nextLayouts = { ...(draftSlide.bodyBlockLayouts || {}) };
//       delete nextLayouts[blockIndex];
//       updateDraft({ bodyBlockLayouts: nextLayouts });
//       return;
//     }
//     if (selectedCanvasBlock.startsWith("visual-item-")) {
//       const itemId = selectedCanvasBlock.replace("visual-item-", "");
//       const nextLayouts = { ...(draftSlide.visualItemLayouts || {}) };
//       delete nextLayouts[itemId];
//       updateDraft({ visualItemLayouts: nextLayouts });
//       return;
//     }
//     if (!isCanvasZoneKey(selectedCanvasBlock)) return;
//     const baseSlide = normalizeSlide({
//       ...draftSlide,
//       canvasLayout: {},
//     });
//     const baseLayout = getCanvasLayout(baseSlide);
//     updateDraft({
//       canvasLayout: {
//         ...(draftSlide.canvasLayout || {}),
//         [selectedCanvasBlock]: baseLayout[selectedCanvasBlock],
//       },
//     });
//   }
...
//         break;
//       case "bring_front":
//         if (selectedCanvasBlock) handleCanvasReorder(selectedCanvasBlock, "forward");
//         break;
//       case "send_back":
//         if (selectedCanvasBlock) handleCanvasReorder(selectedCanvasBlock, "backward");
//         break;
//       case "reset_zone":
//         handleResetSelectedZone();
//         break;
//       case "deselect":
//         setSelectedCanvasBlock(null);
//         break;
...
//       if (!res.ok) {
//         const fallback = data?.error || "Failed to generate deck preview.";
//         const message =
//           res.status === 429
//             ? "The AI provider is rate-limited right now. Please try again in a minute."
//             : fallback;
...
//         ...committedDeck,
//         title: typeof data?.title === "string" ? data.title : committedDeck.title,
//         subtitle: typeof data?.subtitle === "string" ? data.subtitle : committedDeck.subtitle,
//         slides: committedDeck.slides.map((slide, index) => {
//           const generated = nextSlides[index] || {};
//           const prompts = Array.isArray(generated?.imagePrompts) ? generated.imagePrompts : [];
//           const priorVisualItems = Array.isArray(slide.visualItems) ? slide.visualItems : [];
//           const generatedImageMap = data?.selectedIndex === index && data?.slideImages && typeof data.slideImages === "object"
//             ? data.slideImages
//             : {};
//           const nextVisualItems =
//             prompts.length > 0
//               ? prompts.map((prompt: string, promptIndex: number) => ({
//                   id: priorVisualItems[promptIndex]?.id || `generated-${index + 1}-${promptIndex + 1}`,
//                   prompt,
//                   imageData:
//                     generatedImageMap[priorVisualItems[promptIndex]?.id || `generated-${index + 1}-${promptIndex + 1}`] ||
//                     priorVisualItems[promptIndex]?.imageData ||
//                     slide.imageData,
//                 }))
...
//               : slide.body;
//           const nextBullets =
//             Array.isArray(generated?.bullets) && generated.bullets.length > 0
//               ? generated.bullets
//               : slide.bullets;
...
//             body: nextBody,
//             bodyBlocks:
//               typeof nextBody === "string" && nextBody.trim().length > 0
//                 ? [nextBody]
//                 : slide.bodyBlocks,
//             bullets: nextBullets,
//             imagePrompt: prompts.join("\n"),
//             visualItems: nextVisualItems,
//           });
//         }),
//       };
...
//   function handleUpdateBullet(index: number, value: string) {
//     const nextBullets = [...(draftSlide.bullets || [])];
//     nextBullets[index] = value;
//     updateDraft({ bullets: nextBullets });
//   }

//   function handleAddBullet() {
//     const nextBullets = [...(draftSlide.bullets || [])];
//     nextBullets.push(`New bullet ${nextBullets.length + 1}`);
//     updateDraft({ bullets: nextBullets });
//   }

//   function handleRemoveBullet(index: number) {
//     const nextBullets = (draftSlide.bullets || []).filter((_, bulletIndex) => bulletIndex !== index);
//     const nextLayouts = { ...(draftSlide.bulletLayouts || {}) };
//     delete nextLayouts[String(index)];
//     Object.keys(nextLayouts).forEach((key) => {
//       const numeric = Number(key);
...
//       bulletLayouts: nextLayouts,
//     });
//     if (selectedCanvasBlock === `bullet-${index}` || selectedCanvasBlock === `body-block-${index}`) {
//       setSelectedCanvasBlock(null);
//     }
//   }

//   function handleAddImageSlot() {
//     const nextItems = [...visualItemsDraft, { id: createVisualItemId(), prompt: `Image concept ${visualItemsDraft.length + 1}` }];
//     updateDraft({
//       visualItems: nextItems,
//       imagePrompt: nextItems.map((item) => item.prompt).join("\n"),
//     });
...
//     }
//     onDownload(finalDeck);
//   }

//   return (
//     <div className="flex h-screen w-screen flex-col overflow-hidden bg-[#f6f8fc] text-slate-900 dark:bg-[#0b1220] dark:text-slate-100">
//       <div className="border-b border-slate-200 bg-[#f8f9fb] px-4 py-2 shadow-sm dark:border-slate-800 dark:bg-slate-950">
...
//               className="h-9 rounded-lg border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
//             >
...
//               {loading ? "Preparing..." : "Download Edited File"}
//             </Button>
//             {onClose && (
//               <Button
//                 type="button"
...
//         <aside className="flex w-[240px] shrink-0 flex-col border-r border-slate-200 bg-[#f1f3f7] dark:border-slate-800 dark:bg-slate-950">
//           <div className="border-b border-slate-200 px-4 py-4 dark:border-slate-800">
//             <div className="text-sm font-semibold text-slate-900 dark:text-white">
//               {deck.title || "Presentation Alpha"}
//             </div>
//             <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
//               {deck.subtitle || "Edited just now"}
//             </div>
//           </div>
...
//         <section className="relative min-h-0 flex-1 overflow-hidden bg-[#e9eef5] dark:bg-[#0f172a]">
//           <div className="flex h-full items-stretch justify-stretch overflow-hidden">
...
//                 onSelectBlock={setSelectedCanvasBlock}
//                 onLayoutChange={handleCanvasLayoutChange}
//                 onReorderBlock={handleCanvasReorder}
//                 onRemoveBlock={handleRemoveCanvasObject}
...
//         <aside className="flex w-[340px] shrink-0 flex-col border-l border-slate-200 bg-[#fbfcfe] dark:border-slate-800 dark:bg-slate-950">
//           <div className="border-b border-slate-200 px-5 py-5 dark:border-slate-800">
//             <div className="text-lg font-semibold text-slate-900 dark:text-white">Slide Generator</div>
//             <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
//               Describe and refine the slide content for this page.
//             </div>
...
//                         key={option.value}
//                         value={option.value}
//                         disabled={option.requiresSelection && !selectedCanvasBlock}
//                       >
//                         {option.label}
//                       </option>
//                     ))}
...
//                   Live Preview is always on. Changes appear on the canvas immediately.
//                 </div>
//               </div>
//             </div>
//             {previewError ? (
...
//                 Canvas selection: <span className="font-semibold">{selectedCanvasLabel}</span>
//               </div>
//             ) : null}
//           </div>
...
//                   {DEFAULT_CANVAS_ORDER.map((layer) => {
//                     const isActive = isCanvasZoneKey(selectedCanvasBlock) && selectedCanvasBlock === layer;
//                     return (
//                       <div
//                         key={`layer-${layer}`}
...
//                         <button
//                           type="button"
//                           onClick={() => setSelectedCanvasBlock(layer)}
...
//                           <button
//                             type="button"
//                             onClick={() => handleCanvasReorder(layer, "backward")}
...
//                           <button
//                             type="button"
//                             onClick={() => handleCanvasReorder(layer, "forward")}
//                             className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
//                           >
//                             Front
//                           </button>
...
//               {inspectorTab === "content" ? (
//                 <>
//               <div className={selectedCanvasBlock === "title" ? "rounded-2xl border border-blue-200 bg-blue-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10" : ""}>
//                 <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
//                   Slide Title
//                 </div>
//                 <Input
...
//               <div className={selectedCanvasBlock === "bullets" || selectedCanvasBlock?.startsWith("bullet-") ? "rounded-2xl border border-blue-200 bg-blue-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10" : ""}>
//                 <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
//                   Body Bullets
//                 </div>
//                 <div className="space-y-3">
//                   {(draftSlide.bullets || []).map((bullet, bulletIndex) => {
//                     const isSelected = selectedCanvasBlock === `bullet-${bulletIndex}`;
//                     return (
//                       <div
//                         key={`inspector-bullet-${bulletIndex}`}
...
//                           rows={3}
//                           value={bullet}
//                           onChange={(event) => handleUpdateBullet(bulletIndex, event.target.value)}
...
//                   className="mt-3 h-9 rounded-lg border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
//                 >
//                   <Plus className="mr-2 h-4 w-4" />
//                   Add Bullet
//                 </Button>
...
//               <div className={selectedCanvasBlock === "body" || selectedCanvasBlock?.startsWith("body-block-") ? "rounded-2xl border border-blue-200 bg-blue-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10" : ""}>
//                 <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
//                   Body Copy
//                 </div>
//                 <div className="space-y-3">
//                   {(draftSlide.bodyBlocks || [draftSlide.body || ""]).map((block, blockIndex) => (
//                     <div
//                       key={`body-block-${blockIndex}`}
...
//                             );
//                             const nextLayouts = { ...(draftSlide.bodyBlockLayouts || {}) };
//                             delete nextLayouts[String(blockIndex)];
//                             Object.keys(nextLayouts).forEach((key) => {
//                               const numeric = Number(key);
//                               if (Number.isInteger(numeric) && numeric > blockIndex) {
//                                 nextLayouts[String(numeric - 1)] = nextLayouts[key];
//                                 delete nextLayouts[key];
//                               }
//                             });
...
//                         value={block}
//                         onChange={(event) => {
//                           const nextBlocks = [...(draftSlide.bodyBlocks || [draftSlide.body || ""])];
//                           nextBlocks[blockIndex] = event.target.value;
//                           updateDraft({
//                             bodyBlocks: nextBlocks,
//                             body: nextBlocks.join(" ").trim(),
//                           });
...
//                   onClick={() => {
//                     const nextBlocks = [...(draftSlide.bodyBlocks || [draftSlide.body || ""])];
//                     nextBlocks.push(`New body text ${nextBlocks.length + 1}`);
//                     updateDraft({
//                       bodyBlocks: nextBlocks,
//                       body: nextBlocks.join(" ").trim(),
//                     });
...
//               <div className={selectedCanvasBlock === "visual" || selectedCanvasBlock?.startsWith("visual-item-") ? "rounded-2xl border border-blue-200 bg-blue-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10" : ""}>
//                 <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
//                   Image Prompts
//                 </div>
//                 <div className="space-y-3">
//                   {(visualItemsDraft.length > 0 ? visualItemsDraft : [{ id: createVisualItemId(), prompt: "" }]).map(
//                     (item, index) => (
//                       <div
...
//                             type="button"
//                             onClick={() => {
//                               const nextItems = visualItemsDraft.filter((visual) => visual.id !== item.id);
//                               const nextLayouts = { ...(draftSlide.visualItemLayouts || {}) };
//                               delete nextLayouts[item.id];
//                               updateDraft({
//                                 visualItems: nextItems,
//                                 visualItemLayouts: nextLayouts,
//                                 imagePrompt: nextItems.map((visual) => visual.prompt).join("\n"),
//                               });
//                               if (selectedCanvasBlock === `visual-item-${item.id}`) {
//                                 setSelectedCanvasBlock(null);
//                               }
//                             }}
//                             className="text-xs font-medium text-rose-600 hover:text-rose-700 dark:text-rose-300 dark:hover:text-rose-200"
...
//                           value={item.prompt || ""}
//                           onChange={(event) => {
//                             const nextItems = visualItemsDraft.map((visual) =>
//                               visual.id === item.id ? { ...visual, prompt: event.target.value } : visual
//                             );
//                             updateDraft({
//                               visualItems: nextItems,
//                               imagePrompt: nextItems.map((visual) => visual.prompt).join("\n"),
...
//                 </div>
//                 <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
//                   Each image prompt becomes its own image card on the slide.
//                 </div>
//                 <Button
//                   type="button"
//                   variant="outline"
...
//                     </div>
//                     <select
//                       value={draftSlide.fontFamily || "inter"}
//                       onChange={(event) =>
//                         updateDraft({ fontFamily: event.target.value as "inter" | "serif" | "mono" })
//                       }
...
//                     </div>
//                     <select
//                       value={draftSlide.titleSize || "md"}
//                       onChange={(event) =>
//                         updateDraft({ titleSize: event.target.value as "sm" | "md" | "lg" })
//                       }
//                       className="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm uppercase text-slate-700 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
//                     >
...
//                     </div>
//                     <select
//                       value={draftSlide.bodySize || "md"}
//                       onChange={(event) =>
//                         updateDraft({ bodySize: event.target.value as "sm" | "md" | "lg" })
//                       }
//                       className="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm uppercase text-slate-700 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
//                     >
//                       <option value="sm">Small</option>
...
//               <div className={selectedCanvasBlock === "notes" ? "rounded-2xl border border-blue-200 bg-blue-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10" : ""}>
//                 <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
//                   Notes
//                 </div>
//                 <Textarea
//                   rows={4}
//                   value={draftSlide.notes || ""}
...
//                   className="flex-1 rounded-xl border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
//                 >
//                   Reset
//                 </Button>
//                 <Button
...
//                 >
//                   {generatingPreview ? "Generating Deck..." : "Generate Preview"}
//                 </Button>
//               </div>
...
//                 className="w-full justify-start rounded-xl px-0 text-rose-600 hover:bg-transparent hover:text-rose-700 dark:text-rose-300 dark:hover:text-rose-200"
//               >
