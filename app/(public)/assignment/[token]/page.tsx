...
export default function StudentAssignmentPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError("");
      try {
        const res = await fetch(`/api/assignment-share/${encodeURIComponent(token)}`, {
          cache: "no-store",
        });
        const data = await res.json();
        if (!res.ok) {
          setError(data?.error || "Failed to load shared assignment.");
          return;
        }
        setAssignment(data.assignment as SharedAssignment);
        setQuiz(data.quiz as SharedQuiz);
      } catch {
        setError("Failed to load shared assignment.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const badge =
    answeredCount >= 10 ? "Master" : answeredCount >= 5 ? "Challenger" : answeredCount >= 1 ? "Starter" : "Ready";
  const streak = quiz
    ? quiz.questions.reduce((acc, q) => (answers[String(q.id)] ? acc + 1 : acc), 0)
    : 0;
  const elapsedSeconds = sessionStartedAt
    ? Math.max(0, Math.floor((nowTick - sessionStartedAt) / 1000))
    : 0;
  ...
  const expireAndReload = () => {
    void expireAssignmentSession().finally(() => {
  ...
  const submitAssignment = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/assignment-share/${encodeURIComponent(token)}/submit`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-slate-50">
      <Card className="min-h-screen w-full rounded-none border-0 shadow-none">
        <CardHeader className="bg-linear-to-r from-emerald-600 to-cyan-600 text-white">
          <div className="mx-auto w-full max-w-6xl">
            <h1 className="text-2xl font-bold">Student Assignment</h1>
            <p className="text-sm text-emerald-100">
              Complete the assignment and submit your responses.
            </p>
          </div>
        </CardHeader>
        <CardContent className="space-y-5 p-4 md:p-6">
          {loading && <p className="text-sm text-zinc-500">Loading assignment...</p>}
          {!loading && sessionExpired && !result && (
            <Alert variant="destructive">
              <AlertDescription>
                This assignment session expired because the page was refreshed or fullscreen was exited before submission. Reopen the shared link to start again.
              </AlertDescription>
            </Alert>
          )}
          {!loading && error && (
          ...
          {!loading && quiz && assignment && !sessionExpired && (
            <>
              {!quizStarted && !result && (
                  ...
                  <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-900 p-6 text-white shadow-2xl">
                    <div className="space-y-3">
                      ...
                      <div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 p-4 text-sm text-amber-100">
                        Leaving fullscreen or refreshing this page before submission will expire this assignment session. If that happens, you will need to reopen the shared link and start again.
                      </div>
              ...
              <div className="mx-auto w-full max-w-6xl space-y-4">
                <div className="rounded-md border bg-zinc-50 p-3 text-sm">
                  ...
                  <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-zinc-200">
                    <div
                      className="h-full bg-linear-to-r from-emerald-500 to-cyan-500 transition-all"
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          {q.questionType === "worksheet" && (
                            <StudentWorksheetQuestion
                              structure={q.structure?.type === "worksheet" ? q.structure : null}
                              value={answers[String(q.id)] || ""}
                              disabled={Boolean(result)}
                              onChange={(next) =>
                                setAnswers((prev) => ({
                          ...
                          {q.questionType === "matching" && (
                            <StudentMatchingLineRenderer
                              question={q.question}
                              options={safeOptions}
                              structure={q.structure?.type === "matching" ? q.structure : null}
                              value={answers[String(q.id)] || ""}
                              disabled={Boolean(result)}
                              onChange={(next) =>
                                setAnswers((prev) => ({
                ...
                <div className="sticky bottom-0 -mx-4 border-t bg-white/95 px-4 py-3 backdrop-blur md:-mx-6 md:px-6">
                  {!result ? (
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      ...
                      <Button onClick={submitAssignment} disabled={!canSubmit}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-zinc-600">
              Your responses were recorded. You can no longer edit this assignment.
            </p>
            <p className="mt-1 text-xs text-zinc-500">This tab will close in {closeCountdown}s.</p>
      ...
      {showFullscreenWarning && !result && !sessionExpired && (
        <div className="fixed inset-0 z-45 flex items-center justify-center bg-slate-950/72 p-4">
          <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-white p-5 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-zinc-600">
              If you continue after exiting fullscreen, this assignment link will expire and you will need to reopen the shared link to start again.
            </p>
      ...
      {showReloadWarning && !result && !sessionExpired && (
        <div className="fixed inset-0 z-45 flex items-center justify-center bg-slate-950/72 p-4">
          <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-white p-5 shadow-2xl">
            <h3 className="text-lg font-semibold text-zinc-900">Reload will expire this assignment</h3>
            <p className="mt-2 text-sm text-zinc-600">
              Refreshing this page before submission will expire the current assignment session. You will need to reopen the shared link and start again.
            </p>
            <div className="mt-4 flex justify-end gap-2">
