...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl px-6">
      {/* HERO */}
       <section id="hero" className="flex flex-col items-center text-center mt-16 max-w-3xl">
        ...
        <p className="text-lg text-zinc-600 dark:text-zinc-400 mb-8">
          Paste any document, lesson, or article — and let QuizMintAI’s AI create smart, engaging quizzes in seconds.
        </p>

        {/* Input + Button */}
        <form
          className="flex w-full max-w-md gap-2"
          onSubmit={(e) => { e.preventDefault(); generateQuiz(); }}
        >
          <Input
            placeholder="Paste your text or URL..."
            value={text}
            onChange={(e) => setText(e.target.value)}
            disabled={!usageLoaded || loading || usage.remaining <= 0 || !!usage.nextFreeAt}
          />
          <Button
            type="submit"
            ...
            disabled={!usageLoaded || loading || usage.remaining <= 0 || !!usage.nextFreeAt}
          >
            {loading ? "Generating..." : usage.nextFreeAt ? `Wait (${countdown})` : "Generate"}
          ...
          <Alert variant="destructive" className="mt-4 w-full max-w-md">
            <AlertDescription>
              ...
              <div className="flex items-center gap-3">
                ...
                <a href="#pricing" className="text-blue-600 hover:text-blue-800 underline cursor-pointer">
                  See pricing →
                </a>
              </div>
            </AlertDescription>
        ...
        {/* Generated quiz card (same as your original) */}
        {quiz && (
          <Card className="mt-8 w-full max-w-2xl border-zinc-200 dark:border-zinc-800">
            ...
            <CardContent>
              <div className="max-h-96 overflow-y-auto border p-5 rounded-lg bg-linear-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-800 shadow-sm">
                <div ref={quizRef} className="space-y-4 text-left text-base text-zinc-800 dark:text-zinc-200 leading-relaxed">
                  <div className="space-y-6">
                    <h3 className="text-xl font-bold text-blue-600">{quiz.title}</h3>
                    <p className="text-zinc-600 dark:text-zinc-300 italic">{quiz.instructions}</p>
                    <div className="space-y-5">
                      {quiz?.questions?.map((q: any, i: number) => (
                        <div key={i} className="p-3 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
        ...
        <p className="text-sm text-zinc-500 mt-3">
          Free quizzes used: {usage.count}/3 {usage.nextFreeAt && <span className="text-blue-600"> (Next in {countdown})</span>}
        </p>
      </section>

      {/* FEATURES */}
      <section className="grid sm:grid-cols-3 gap-6 mt-24 w-full" aria-label="Features">
        <Card>
          <CardHeader>
          ...
          <CardContent>
            Generate multiple-choice, true/false, or short-answer quizzes from any content in seconds.
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>Detects context, difficulty, and key points for balanced questions.</CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>Export to Google Forms, PDF, or embed quizzes in your LMS.</CardContent>
        </Card>
      </section>
